...
if (obj.summationExponent == 1)
    if (max(abs(unitEquivContrastResponse1-unitEquivContrastResponse2)) > 1e-10 & min(unitEquivContrastResponse1) >= 1e-6)
...
if (max(abs(checkContrastResponse - obj.criterionResp)) > 1e-6)
